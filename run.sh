...
feats_dir= #give the feature dir where you store your feats, it must includes {tr, cv, tt}_{inputs, labels} dirctories
copy_labels=false
...
if [ $step -le 1 ] ; then
    ...
    for x in tr, cv, tt; do 
        ...
        compute-cmvn ark:$feats_dir/${x}_inputs/feats.txt $feats_dir/${x}_${y}/cmvn.ark &
    done
    wait 
fi
